...
public class ReadMoreTextView: UITextView {
    ...
    public override func touchesEnded(_ touches: Set<UITouch>, with event: UIEvent?) {
        if let point = touches.first?.location(in: self) {
            ...
            _ = touchHandler(point)
        }
        super.touchesEnded(touches, with: event)
    }

    public func touchHandler(_ point: CGPoint) -> Bool {
        if let isReadLess = pointIsInReadMoreOrReadLessTextRange(point: point) {
            if isReadLess {
                onClickOnReadLess == nil ? showLessText() : onClickOnReadLess?()
            } else {
                onClickOnReadMore == nil ? showMoreText() : onClickOnReadMore?()
            }
            return true
        }
        return false
